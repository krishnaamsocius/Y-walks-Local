...
class AuthSessions(Session):

    @route()
    def authenticate(self, *args, **kw):
        response = super(AuthSessions, self).authenticate(*args, **kw)
        try:
            if response.get('uid'):
                _logger.info("🔑 Login attempt for user ID %s", response.get('uid'))
                user = request.env['res.users'].sudo().browse(response.get('uid'))

                if not user.exists():
                    return {"status": "error", "message": "User not found"}

                #  If user is already logged in → return status 'logged_out'
                if user.is_logged_in:
                    _logger.warning("⚠️ User %s already logged in, returning logged_out status", user.name)
                    return {
                        "status": "logged_out",
                        "message": f"User {user.name} is already logged in."
                    }

                #  Proceed with normal login flow
                user.generate_api_rest_key()
                user.sudo().write({'is_logged_in': True})
                _logger.info(" User %s marked as logged in", user.name)
            ...
            _logger.exception("❌ Error during user authentication:")
...
class RestApi(Controller):
    ...
    def search_read(self, _api_version, _api_name, _api_path, **kwargs):
        ...
        if _api_name == 'getOfcTransfer' and _api_name == 'getPollLocations':
            kwargs['limit'] = 900
        http_request = request.httprequest
        port = http_request.environ.get('SERVER_PORT')
        if int(port) == 8069 or int(port) == 8070 or int(port) == 8071 or int(port) == 8072:
            if _api_name == 'getLostReport':
                kwargs['limit'] = 900
                kwargs['order'] = 'id asc'
            if _api_name == 'getDetailedOperationLines':
                extra_condition = [('qty_done', '!=', 0)]
                kwargs['domain'] += extra_condition
    ...
    def create(self, _api_version, _api_name, _api_path, **kwargs):
        ...
        port = http_request.environ.get('SERVER_PORT')
        if int(port) == 8069 or int(port) == 8070 or int(port) == 8071 or int(port) == 8072:
            if model_obj._name == 'stock.move':
                return True
        data = model_obj.create(_api_path._post_treatment_values(kwargs)).id
