...
class api_management():
    def __call__(self, func):
        ...
        def wrapper(*args, **kwargs):
            ...
            version = request.env['api.rest.version'].search([('name','=',str(_api_version))], limit=1)
            if version and version.active_session:
                try:
                    security.check_session(request.session, request.env)
                except Exception:
                    return make_error(
                        1007, 'Odoo Session Expired',
                        _('Odoo Session Expired'), status=401)
            # session auth ends
